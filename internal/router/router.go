...
func NewRouter(databases *db.DB) *gin.Engine {
	...
	v1 := r.Group("/api/v1")
	{
		...
		authV1 := v1.Group("/")
		authV1.Use(middleware.DeviceMiddleware())
		{
			// Timecode routes
			...
			favourite := authV1.Group("/favourite")
			{
			...
			collection := authV1.Group("/collection")
			{
